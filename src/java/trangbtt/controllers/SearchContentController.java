    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (!searchValue.equals("")) {
                ...
                List<ArticleDTO> result = dao.searchActicleByContent(searchValue, iCurrentPage, 20);
                if(result == null){
                    request.setAttribute("NLIST", "No content list");
                }
                loadPage.setListActicle(result);
                int count = dao.countSearchTotalPageAdminContent(searchValue); 
                if (count % 20 == 0) { 
                    pageTotal = count / 20;
            ...
            log("ERROR at SearchContentController" + e.getMessage());
        } finally {
            request.getRequestDispatcher("searchByContent.jsp").forward(request, response);
        }
    }
